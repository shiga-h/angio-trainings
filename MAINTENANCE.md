...
3. 届いた Excel ファイルを **ファイル選択欄にドラッグ**
   - **ファイル名は変えなくてOK**（`20260705_2026年度…xlsx` のような日付接頭辞ありでもそのまま）
...
GitHub Actions が自動起動（.github/workflows/convert.yml）
  - 今回追加/変更された xlsx を検出
  - convert.py で data.json を再生成
  - 対象外の古い xlsx を自動削除
  - 上記を1つの commit にまとめて自動 push
   ↓ 30秒〜1分
Vercel が再デプロイ
   ↓ 30秒〜1分
サイトに反映（合計1〜2分）
...
Actions は「今回変更された xlsx」を data.json のソースとして扱い、**それ以外の xlsx をリポジトリから自動削除**します。これにより:

- 事前に古いファイルを手で消す必要なし
- リポジトリ内には常に「最新の xlsx が1つだけ」ある状態が維持される
- ファイル名が変わっても自動的に切り替わる
...
Excel の中身については以下を守ってください。GitHub Actions は Excel の構造が壊れていると失敗します（ファイル名は自由）。

- シート構成の対応:
  - `研究会一覧` シートがあればそれを使う（統合版）
  - なくても `4月`〜`3月` のような月名シートが2つ以上あれば **全部読んで自動統合**
  - シートが1つだけの場合はシート名を問わずそれを使う
  - どれにも該当しないと Actions がエラーで停止（ログにシート名一覧が出るので対応可能）
...
[GitHub Actions: convert.yml]
  - Python 3.11 セットアップ、openpyxl インストール
  - 今回追加/変更された xlsx を検出
  - python3 convert.py <その xlsx> を実行
  - 対象外の古い xlsx を git rm で削除
  - data.json + 削除分を1つの commit にまとめて自動 push
     ↓
[Vercel: main ブランチを監視]
